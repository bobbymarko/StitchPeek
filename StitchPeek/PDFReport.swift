...
enum PDFReport {
    ...
    private static func detailRows(design: Design, filename: String) -> [(label: String, value: String)] {
        var rows: [(String, String)] = [
            ...
            ("Thread (est.)", format(lengthMM: design.threadLengthMM)),
            ("Bobbin thread (est.)", format(lengthMM: design.estimatedBobbinLengthMM)),
            ("Run time (est. @ \(Int(stitchesPerMinute)) spm)",
             format(duration: design.estimatedRunTime(stitchesPerMinute: stitchesPerMinute)))
        ]
    ...
    private static func drawColorBlocks(
        ...
        for (index, block) in design.blocks.prefix(limit).enumerated() {
            ...
            _ = draw(
                "\(block.stitchCount.formatted()) stitches  ·  \(block.color.hexString)  ·  \(format(lengthMM: block.threadLengthMM))",
                font: .systemFont(ofSize: 9.5),
                color: .secondaryLabelColor,
    ...
    /// Draws a single line and returns the height it consumed, so callers can walk down
    /// the page without tracking font metrics themselves.
    @discardableResult
    private static func draw(
        ...
        // topLeft is the top of the line; AppKit draws from the bottom-left in a y-up context.
    ...
    private static func format(lengthMM: Double) -> String {
        if lengthMM >= 1000 { return String(format: "~%.1f m", lengthMM / 1000) }
        if lengthMM >= 10 { return String(format: "~%.0f cm", lengthMM / 10) }
        return String(format: "~%.0f mm", lengthMM)
    }
